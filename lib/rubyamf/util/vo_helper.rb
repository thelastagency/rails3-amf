module RubyAMF
  module VoHelper
    ...
    require 'app/configuration' if Rails::VERSION::MAJOR < 3 #fosrias backward compatibility # cant put this at the top because VoHash has to be instantiated for app/configuration to work
    class VoUtil
      ...
      def self.set_value(obj, key, value)
        if obj.kind_of?(ActiveRecord::Base)
          ...
          raise RUBYAMFException.new(RUBYAMFException.VO_ERROR, "Argument, #{obj}, is not an ActiveRecord::Base, ActiveResource::Base or a Hash.")
        end
      end
      ...
      def self.get_vo_hash_for_outgoing(obj)
        new_object =  Rails::VERSION::MAJOR < 3 ? VoHash.new : {} #Use VoHash because one day, we might do away with the class Object patching. fosrias: use a hash for rails3
        ...
        if map = ClassMappings.get_vo_mapping_for_ruby_class(obj.class.to_s)
          ...
          new_object._explicitType = map[:actionscript] if Rails::VERSION::MAJOR < 3 #fosrias backward compatibility. # Aryk: This only works on the Hash because rubyAMF extended class Object to have this accessor, probably not the best idea, but its already there.
          ...
          new_object._explicitType = obj.class.to_s  if Rails::VERSION::MAJOR < 3 #fosrias backward compatibility
        ...
        if  Rails::VERSION::MAJOR < 3 #fosrias Not DRY but faster than redundantly checking version on each iteration.
          ...
          instance_vars.each do |var| # this also picks up the eager loaded associations, because association called "has_many_assoc" has an instance variable called "@has_many_assoc"
            attr_name = var[1..-1]
            attr_name.to_camel! if ClassMappings.translate_case
            new_object[attr_name.to_sym] = obj.instance_variable_get(var)
          end
          methods.each do |method|
            attr_name = method.dup
            attr_name.to_camel! if ClassMappings.translate_case
            new_object[attr_name.to_sym] = obj.send(method)
          end
        end
        new_object
      rescue Exception => e
