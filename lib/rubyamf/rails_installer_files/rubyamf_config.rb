require 'app/configuration' if Rails::VERSION::MAJOR < 3 #fosrias backward compatibility

module RubyAMF
  module Configuration
    #set the service path used in all requests
    # RubyAMF::App::RequestStore.service_path = File.expand_path(RAILS_ROOT) + '/app/controllers' if Rails::VERSION::MAJOR < 3
    ...
    #
    # => Optional Rails 3+ specific value object properties:
    # :include        # Specify which associations to read on an active record (only applies to active records)
    # :only           # Specifically which attributes to include in the serialization
    # :except         # Specifically which attributes to exclude from the serialization (use either :only or :except, not both)
    #
    ...
    # or for ":associations." In Rails 3+ you can define a scope for ":include", "only", and/or ":except". You're mapping would look something like this.
    ...
    # => Date Conversion
    # Incoming dates from Flash by default are Time objects, this can convert to DateTime if needed
    ...
    # If set to true, arguments from Flash/Flex will come in to the controllers as params[0], params[1], etc.. This is especially useful if you are sending huge objects
    # from Flex into Ruby so it doesn't eat up all your output window with outputting the params in the controller/action header information while in dev mode.
